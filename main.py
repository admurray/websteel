...
def getpagelinks(page, filetype=IMAGE):
    parsed_uri = parse.urlparse(page)
...
def get_pages(base_url,links_list, output_dir):
    for link in links_list:
        page_url = parse.urljoin(base_url, link)
...
def get_page_data(page_url, output_dir, base_url=None, kill=False):
    ...
    #page_url = page_url.replace('thumb', 'full')
    ...
    if not response.status_code == 404 and len(response.content) >= 1000:
...
def get_direct_links(page_url): #: http://xyz.com/a= &b= &c=img.jpg
    for each in generate_links(page_url, [(2, 1, 25)]):
        get_page_data(each, '~/Downloads/')


if __name__ == '__main__':
    link = ''
    get_direct_links(link)

    link2 = ''
    #get_forum_docs(forum_link=link2, start_page=1, last_page=36)
    '''
    Scenarios : 
